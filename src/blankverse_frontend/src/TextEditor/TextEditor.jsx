...
export default function TextEditor(props) {
  ...
  window.addEventListener("beforeunload", async function (e) {
    ...
    await blankverse_backend.removeFromActive(documentId, myPrincipal);
  });
  ...
  class MyPeer {
    constructor(recipient, myPrincipal) {
      ...
      this.peer.on("signal", async (data) => {
        if (this.recipient.length > 0) {
          await blankverse_backend.updateCurrentPeers(
            this.myPrincipal,
            this.recipient,
            data.type,
            data.sdp
  ...
  useEffect(() => {
    async function initialRun() {
      var currentActive = await blankverse_backend.getActiveUsers(documentId);
      if (!currentActive.includes(myPrincipal)) {
        await blankverse_backend.addToCurrentUsers(documentId, myPrincipal);
      }
    }

    async function activeUserUpdate() {
      // get active users from motoko
      var peersActive = await blankverse_backend.getActiveUsers(documentId);
      var foundMe = false;
  ...
  useEffect(() => {
    async function connectionRequests() {
      ...
      var request = await blankverse_backend.getConnectionRequest(myPrincipal);
  ...
  useEffect(() => {
    ...
    async function saveDocInterval() {
      await blankverse_backend.updateDocContents(
        documentId,
  ...
  useEffect(() => {
    const sendDoc = async () => {
      var head = await blankverse_backend.getFirst(documentId);
  ...
  useEffect(() => {
    if (quillLoaded) {
      const retrieveDocContent = async () => {
        var docCon = await blankverse_backend.getDocContents(documentId);
